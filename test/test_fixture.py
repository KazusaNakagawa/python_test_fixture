...
class TestFixture(unittest.TestCase):
    ...
    def setUp(self, open_file='../data/response.json', open_file2='../data/user.json') -> None:
        ...
        self.response_user_data = self._read_response_json_file(open_file=open_file2)
    ...
    def _tmp_tow_nest_success_response_data(self, idx=0, first_key='data', second_key='user'):
        ...
        """
        for key in self.response_user_data[idx][first_key][second_key].keys():
            with self.subTest(key=key):
                self.assertEqual(
                    self.request_user[idx][first_key][second_key][key],
                    self.response_user_data[idx][first_key][second_key][key]
                )
    ...
    def test_response_success3(self):
        """ テストファイル, 比較項目テスト """
        self._tmp_tow_nest_success_response_data(idx=0, first_key='data', second_key='user')
